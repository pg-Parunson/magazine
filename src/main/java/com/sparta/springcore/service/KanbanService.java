...
import com.sparta.springcore.dto.BucketDto;
import com.sparta.springcore.dto.BucketInfoDto;
import com.sparta.springcore.dto.CardInfoDto;
import com.sparta.springcore.model.KanbanBucket;
import com.sparta.springcore.model.Project;
import com.sparta.springcore.repository.KanbanBucketRepository;
import com.sparta.springcore.repository.KanbanCardRepository;
...
//        String pjId = bucketInfoDto.getPjId();
//        Project project = projectRepository.findByPjId(pjId); // 프로젝트 ID 로 프로젝트 조회
        String title = bucketInfoDto.getTitle();
        int position = bucketInfoDto.getPosition();

        // 2. 새 버킷 객체를 생성한다.
        KanbanBucket bucket = new KanbanBucket(null, title, position, null);

        // 3. 새로 생성한 버킷을 Repository 를 이용하여 DB에 저장한다.
        kanbanBucketRepository.save(bucket);
